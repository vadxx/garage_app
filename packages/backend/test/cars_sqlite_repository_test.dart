...
void main() {
  ...
  group('recalculateCarStats', () {
    ...
    CarStats loadStats(int carId) => repo.loadCarStats(carId);
    ...
    test('with no works: totalSpent=0, lastOilChangeKm=-1, topCategory=-1', () {
      ...
      final car = repo.load().first;
      repo.saveCarStats(
        CarStats(carId: car.id, totalSpent: 999, lastOilChangeKm: 999),
      );

      repo.recalculateCarStats(car.id);

      final stats = loadStats(car.id);
    ...
    test('sums costs and finds top category', () {
      ...
      final car = repo.load().first;
      repo.saveCarStats(
        CarStats(carId: car.id, totalSpent: 0, lastOilChangeKm: 0),
      );

      worksRepo.insert(
        makeWork(carId: car.id, category: 0, cost: 150, description: 'Oil'),
      );
      worksRepo.insert(
        makeWork(carId: car.id, category: 1, cost: 200, description: 'Fuel'),
      );
      worksRepo.insert(
        makeWork(carId: car.id, category: 5, cost: 350, description: 'Repair'),
      );
    ...
    test('ignores works from other cars', () {
      ...
      final car2 = cars[1];
      repo.saveCarStats(
        CarStats(carId: car1.id, totalSpent: 0, lastOilChangeKm: 0),
      );
      repo.saveCarStats(
        CarStats(carId: car2.id, totalSpent: 0, lastOilChangeKm: 0),
      );

      worksRepo.insert(
        makeWork(carId: car1.id, cost: 500, description: 'Car1 work'),
      );
      worksRepo.insert(
        makeWork(carId: car2.id, cost: 300, description: 'Car2 work'),
      );

      repo.recalculateCarStats(car1.id);

      expect(loadStats(car1.id).totalSpent, 500);
      expect(loadStats(car1.id).topCategory, 0); // only oil category
      // car2's stats are lazy-recalculated when loaded, discovering its work
      expect(loadStats(car2.id).totalSpent, 300);
      expect(loadStats(car2.id).topCategory, 0);
    });
    ...
    test(
      'with non-oil works only: sentinels for oil stats, topCategory from spend',
      () {
        repo.insert(
          Car(
            id: 0,
        ...
        worksRepo.insert(
          makeWork(carId: car.id, category: 1, cost: 100, description: 'Fuel'),
        );
        worksRepo.insert(
          makeWork(
            carId: car.id,
    ...
    test('topCategory uses category with highest total cost', () {
      ...
      worksRepo.insert(
        makeWork(carId: car.id, category: 1, cost: 250, description: 'Fuel A'),
      );
      worksRepo.insert(
        makeWork(carId: car.id, category: 1, cost: 350, description: 'Fuel B'),
      );
      // category 5 has one work totalling 500
      worksRepo.insert(
        makeWork(carId: car.id, category: 5, cost: 500, description: 'Repair'),
      );
